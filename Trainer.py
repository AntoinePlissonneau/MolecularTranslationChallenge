...
from models_InChI import DLChI
...
class Trainer:
    ...
    def __init__(self, params):
        ...
        if checkpoint is False:
            ...
            # Model parameters
            encoded_image_size= params['encoded_image_size']
            dropout = params['dropout']
            output_dim = params['output_dim']

            # Training parameters
            self.start_epoch = params['start_epoch']
            self.epochs_since_improvement = params['epochs_since_improvement']  # keeps track of number of epochs since there's been an improvement in validation BLEU

            self.best_mse = params['best_mse']  # MSE score right now
            fine_tune = params['fine_tune']  # fine-tune

            self.model = DLChI(output_dim, encoded_image_size, dropout)
            self.model.fine_tune(fine_tune)
            self.model_optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, self.model.parameters()),lr = self.model_lr)
    ...
    def predict_batch(self, image):
        ...
        """
        Performs one prediction on batch.
        :param image: torch vector of image (batch_size, 3, size, size).
        """
        ...
        print('Generating molecular formula from predictions')

        res = []
        for i in tqdm(range(batch)):
            res.append(mol_fb(scores[i], atom))
